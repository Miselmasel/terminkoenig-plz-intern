...
var allLayers = {};
var centroids = {};
// Label-Canvas: PLZ- und Städte-Beschriftungen werden in EIN Canvas gezeichnet
// statt als hunderte einzelne DOM-Marker – der DOM-Neuaufbau bei jedem Zoom
// war der größte Performance-Killer beim Zoomen.
map.createPane('labelPane');
map.getPane('labelPane').style.zIndex = 620;
map.getPane('labelPane').style.pointerEvents = 'none';
var labelCanvas = document.createElement('canvas');
L.DomUtil.addClass(labelCanvas, 'leaflet-zoom-hide');
map.getPane('labelPane').appendChild(labelCanvas);
var multiMode = false;
var multiPoints = [];
...
function redrawLabelCanvas() {
  var size = map.getSize();
  var topLeft = map.containerPointToLayerPoint([0, 0]);
  L.DomUtil.setPosition(labelCanvas, topLeft);
  if (labelCanvas.width !== size.x) labelCanvas.width = size.x;
  if (labelCanvas.height !== size.y) labelCanvas.height = size.y;
  ...
  // Dropdown-Anzeige synchron halten (Zoom 11 drückt die Untergrenze)
  var selEl = document.getElementById('cityMinPop');
  if (selEl) {
    var displayValue = cityThr < cityMinPop ? closestCityOption(cityThr) : cityMinPop;
    if (String(displayValue) !== selEl.value) selEl.value = String(displayValue);
  }
  var cityPts = [];
  ...
  STAEDTE.forEach(function(c) {
    ...
    var thr = cityThr;
    if (isNRWArea(lat, lon)) thr = Math.max(thr, cityPopThresholdNRW(zoom));
    if (pop < thr) return;
    ...
    ctx.beginPath();
    ctx.arc(pt.x, pt.y, pop > 500000 ? 5 : pop > 100000 ? 4 : 3, 0, Math.PI * 2);
    ...
    ctx.strokeStyle = '#fff';
    ctx.lineWidth = 3;
    ctx.strokeText(name, pt.x + 6, pt.y - 6);
    ctx.fillText(name, pt.x + 6, pt.y - 6);
  });

  // ---- PLZ-Nummern (mit Kollisionsvermeidung wie bisher) ----
  if (zoom < 7) return;
...
var _labelRedrawTimer = null;
function scheduleLabelRedraw() {
  clearTimeout(_labelRedrawTimer);
  _labelRedrawTimer = setTimeout(redrawLabelCanvas, 120);
}
// Wrapper, damit alle bestehenden Aufrufstellen weiter funktionieren
function addLabels() { scheduleLabelRedraw(); }
function updateCityLayer() { scheduleLabelRedraw(); }

map.on('moveend zoomend resize', scheduleLabelRedraw);
...
function captureMapToCanvas() {
  return new Promise(function(resolve, reject) {
    ...
    Promise.all(loadPromises).then(function() {
      // Canvas-Renderer-Ausgabe (Statusfarben der PLZ-Polygone) übernehmen —
      // der SVG-Schritt darunter erfasst nur die SVG-Overlays
